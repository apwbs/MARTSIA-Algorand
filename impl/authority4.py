...
conn = sqlite3.connect('files/authority4/authority4_database.db')
x = conn.cursor()
...
def save_authorities_names(api, process_instance_id):
    ...
    print(os.system('python3.11 blockchain/BoxContract/BoxContractMain.py %s %s %s %s' % (
        authority4_private_key, method, app_id_box, authorities_name_padded)))
...
def initial_parameters_hashed(groupObj, process_instance_id):
    ...
    x.execute("INSERT OR IGNORE INTO h_values VALUES (?,?,?)", (process_instance_id, h1_4, h2_4))
    conn.commit()

    method = 'read_box'
    result = subprocess.run(['python3.11', 'blockchain/BoxContract/BoxContractMain.py', authority4_private_key, method,
                             app_id_box], stdout=subprocess.PIPE).stdout.decode('utf-8')
    authorities = result[:47]
    hashed = authorities + h1_4 + ',' + h2_4 + '#'
    padding = '0' * 275
    hashed_padded = hashed + padding

    method = 'put_box'
    print(os.system('python3.11 blockchain/BoxContract/BoxContractMain.py %s %s %s %s' % (
        authority4_private_key, method, app_id_box, hashed_padded)))

    g1_4_bytes = groupObj.serialize(g1_4)
    g2_4_bytes = groupObj.serialize(g2_4)

    x.execute("INSERT OR IGNORE INTO g_values VALUES (?,?,?)", (process_instance_id, g1_4_bytes, g2_4_bytes))
    conn.commit()
...
def initial_parameters(process_instance_id):
    ...
    result = subprocess.run(['python3.11', 'blockchain/BoxContract/BoxContractMain.py', authority4_private_key, method,
                             app_id_box], stdout=subprocess.PIPE).stdout.decode('utf-8')
    elements = g1_4_bytes.decode('utf-8') + ',' + g2_4_bytes.decode('utf-8') + '#'
    ...
    method = 'put_box'
    print(os.system('python3.11 blockchain/BoxContract/BoxContractMain.py %s %s %s %s' % (
        authority4_private_key, method, app_id_box, hashed_elements_padded)))
...
def generate_public_parameters(groupObj, maabe, api, process_instance_id):
    ...
    hashes1 = [g1g2_1_hashed_split[0], g1g2_2_hashed_split[0], g1g2_3_hashed_split[0], h1]
    hashes2 = [g1g2_1_hashed_split[1], g1g2_2_hashed_split[1], g1g2_3_hashed_split[1], h2]
    ...
    method = 'put_box'
    print(os.system('python3.11 blockchain/BoxContract/BoxContractMain.py %s %s %s %s' % (
        authority4_private_key, method, app_id_box, hashed_elements_pp_padded)))
...
def generate_pk_sk(groupObj, maabe, api, process_instance_id):
    ...
    x.execute("INSERT OR IGNORE INTO public_keys VALUES (?,?,?)", (process_instance_id, hash_file, pk4_bytes))
    conn.commit()
    ...
    method = 'put_box'
    print(os.system('python3.11 blockchain/BoxContract/BoxContractMain.py %s %s %s %s' % (
        authority4_private_key, method, app_id_box, hashed_elements_pp_pk)))
